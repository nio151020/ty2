...
train = load_json(os.path.join(prefix, 'near_data', 'train_set.json'))
dev = load_json(os.path.join(prefix, 'near_data', 'dev_set.json'))
...
def train_epoch(_epoch):
    ...
    for _, data in tqdm(enumerate(train_loader)):
        ...
        loss.backward()
        optimizer.step()
    torch.save(model, os.path.join(model_prefix, 'model_roberta-xlarge{}.pkl'.format(_epoch + 1)))
...
def validate():
    ...
    if f1_score_micro > best_f1:
        best_f1 = f1_score_micro
        # torch.save(model, os.path.join(model_prefix, 'model_bert-base.pkl'))
        print(f"=========best performer here=============")
