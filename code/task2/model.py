...
class MyModel(object):
    ...
    def __init__(self, 
        ...
        with tf.variable_scope('bio_projection'):
            logits_bio = tf.layers.dense(bert_outputs, vocab_size_bio) # B * S * V(BIO类别)
        ...
        with tf.variable_scope('attr_projection'):
            logits_attr = tf.layers.dense(bert_outputs, vocab_size_attr) # B * S * V(attr类别)
        ...
        with tf.variable_scope('type_projection'):
            logits_type = tf.layers.dense(bert_outputs, vocab_size_type) # B * S * V(attr类别)
            probs_type = tf.nn.softmax(logits_type, axis=-1)
            preds_type = tf.argmax(probs_type, axis=-1, name="preds_type") # B * S

        self.outputs = (preds_bio, preds_attr, preds_type)
        ...
        with tf.variable_scope('loss'):
            ...
            loss = loss_bio + loss_attr + loss_type # B
        ...
        with tf.variable_scope('opt'):
            ...
            opt1 = tf.train.AdamOptimizer(2e-5)
